...
function int(node) {
	...
	//it was number
	if (num != null) {
		return Descriptor(`ti.i32(${parseFloat(num)}`, { type: 'int', complexity: 0 });
	}

	//it was string
	return Descriptor(`ti.i32(${result})`, { type: 'int', complexity: result.complexity });
}
int.type = 'int';
...
function createVec3(type, vecType) {
	...
	function vec3(x, y, z) {
		...
		// LS CHANGE - Spread and slice unsupported by ti.
...
function mat3(v0, v1, v2) {
	...
	if (v0.type === 'mat4') {
		...
		return Descriptor(`${v0}.filter((x, i) => i % 4 !== 3 && i < 12;)`, {
