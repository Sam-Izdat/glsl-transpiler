...
GLSL.prototype.transforms = {
	...
	function: function (node) {
		...
		// LS CHANGE - for some reason a semicolon is inserted after `function x(y) { }` which crashes ti
		//   changing to arrow func to duck search for semicolon.
		//create function body
		result += `let ${name} = (${args}) => {\n`

		// //create function body
		// result += `function ${name} (${args}) {\n`

		// LS CHANGES - Handled by ti.
	...
	'do-while': function (node) {
		...
		var cond = this.process(node.children[1])
		// LS CHANGE - do while presumably unsupported in ti
	...
	ternary: function (node) {
		...
		// LS CHANGE - ternary operations unsupported in ti at this time
		// return Descriptor(`${cond} ? ${a} : ${b}`, { type: a.type })
		return Descriptor(`if (${cond}) { ${a} } else { ${b} }`, { type: a.type })
	},
